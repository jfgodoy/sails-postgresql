  ...
  var adapter = {
    ...
    create: function(connectionName, table, data, cb) {
      spawnConnection(connectionName, function __CREATE__(client, cb) {
        ...
        var attributes = utils.mapAttributes(data, collection.definition),
        ...
        // Build Query
        var query = 'INSERT INTO ' + tableName + ' (' + columnNames + ') values (' + paramValues + ') RETURNING ' + utils.buildReturningStatement(originalTable, attributes.keys, collection.definition);
      ...
      spawnConnection(connectionName, function __CREATE_EACH__(client, cb) {
        ...
        async.each(records, function(data, cb) {
          ...
          var query = 'INSERT INTO ' + tableName + ' (' + columnNames +
            ') values (' + paramValues + ') RETURNING ' + utils.buildReturningStatement(table, attributes.keys, collection.definition) + ';';
