...
Query.prototype.update = function(table, criteria, data) {
  ...
  var attributes = utils.mapAttributes(data, this._schema);
  ...
  this._query += ' RETURNING ' + utils.buildReturningStatement(table, attributes.keys, this._schema);
...
Query.prototype.destroy = function(table, criteria) {
  ...
  // Add RETURNING clause
  this._query += ' RETURNING ' + utils.buildReturningStatement(table, Object.keys(this._schema), this._schema);
...
Query.prototype._build = function(table, criteria) {
  ...
  if (hop(criteria, 'limit')) orderedCriteria.limit = criteria.limit;
  if (hop(criteria, 'skip')) orderedCriteria.skip = criteria.skip;
...
Query.prototype.buildSubQuery = function buildSubQuery(table, criteria) {
  ...
  tableName = utils.escapeName(table);
  ...
  namedPk = tableName + '.' + utils.escapeName(pk);
...
Query.prototype.joins = function(joins) {
  ...
  joins.forEach(function(join) {
    ...
    alias = utils.escapeName(join.alias.toLowerCase() + '_' + tableName);

    // Escape Table Name
    tableName = utils.escapeName(tableName);

    // Build Full Key Names
    foreignKey = alias + '.' + utils.escapeName(join.childKey);
    parentKey = utils.escapeName(join.parent) + '.' + utils.escapeName(join.parentKey);
    ...
    joins.forEach(function(otherJoin) {
      if(otherJoin.child === join.parent) {
        parentKey = utils.escapeName(otherJoin.alias.toLowerCase() + '_' + otherJoin.child.toLowerCase());
        parentKey += '.' + utils.escapeName(join.parentKey);
      }
    });
  ...
  function processWhere(options) {
    Object.keys(options).forEach(function(key) {
      switch(key.toLowerCase()) {
        ...
        case 'intersects':
          console.log(options)
          var p1 = options[key][0];
          var p2 = options[key][1];
          ...
          self._query += 'ST_Intersects(' + p1 + ',' + p2 + ') ';
          return;
...
Query.prototype.operators = function() {
  ...
  var sql = {
    ...
    like: function(table, parent, key, options, comparator) {
      ...
      if(self._schema[parent] && self._schema[parent].type === 'text') caseSensitive = false;
    ...
    in: function(table, key, options) {
      ...
      // Check if key is a string
      if(self._schema[key] && self._schema[key].type === 'text') caseSensitivity = false;

      // Check case sensitivity to decide if LOWER logic is used
      if(!caseSensitivity) {
        key = 'LOWER(' + utils.escapeName(table) + '.' + utils.escapeName(key) + ')';
        self._query += key + ' IN (';
      } else {
        self._query += utils.escapeName(table) + '.' + utils.escapeName(key) + ' IN (';
      }
...
function processCriteria(table, parent, value, combinator, caseSensitive) {
  ...
  function expandCriteria(obj) {
    ...
    Object.keys(obj).forEach(function(key) {
      ...
      if(!caseSensitive && _.isString(obj[key])) {
        _param = 'LOWER(' + utils.escapeName(table) + '.' + utils.escapeName(parent) + ')';
        obj[key] = obj[key].toLowerCase();
      } else {
        _param = utils.escapeName(table) + '.' + utils.escapeName(parent);
      }
  ...
  if(!caseSensitive && typeof value === 'string') {
    ...
    // ADD LOWER to parent
    parent = 'LOWER(' + utils.escapeName(table) + '.' + utils.escapeName(parent) + ')';
    value = value.toLowerCase();

  } else {
    // Escape parent
    parent = utils.escapeName(table) + '.' + utils.escapeName(parent);
  }
...
Query.prototype.group = function(table, options) {
  ...
  options.forEach(function(key) {
    self._query += utils.escapeName(table) + '.' + utils.escapeName(key) + ', ';
  });
