...
utils.buildSchema = function(obj) {
  ...
  Object.keys(obj).forEach(function(key) {
    ...
    var str = [
      '"' + key + '"',                        // attribute name
      utils.sqlTypeCast(attr.type, attr),     // attribute type
...
utils.mapAttributes = function(data, schema) {
  ...
  Object.keys(data).forEach(function(key) {
    keys.push('"' + key + '"');
    values.push(utils.prepareValue(data[key], key, schema));
    params.push('$' + i);
    i++;
  });
...
utils.prepareValue = function(value, key, schema) {
  ...
  if (schema[key].type.indexOf('geometry') === 0) {
    try{
      var json = (typeof value === 'string') ? JSON.parse(value) : value;
      var wkt = require('terraformer-wkt-parser');
      return 'SRID=4326;' + wkt.convert(json);
    }catch(err){
      return value;
    }
  }
...
utils.sqlTypeCast = function(type, attr) {
  switch(type.toLowerCase()) {
    ...
    case 'geometry':
      return 'GEOMETRY("' + (attr.geometry || 'Geometry') +'", 4326)';
...
utils.buildReturningStatement = function(table, columns, schema){
  var str = '';
  // Add all the columns to be selected
  columns.forEach(function(column) {
    column = column.replace(/["']/g, "")
    if (schema[column] && schema[column].type.indexOf('geometry') === 0) {
      str += 'ST_asGeoJSON(' + utils.escapeName(table) + '.' + utils.escapeName(column) + ') as ' + utils.escapeName(column) + ', ';
    } else {
      str += utils.escapeName(table) + '.' + utils.escapeName(column) + ', ';
    }
  });
...
utils.buildSelectStatement = function(criteria, table, attributes, schema) {
  ...
  selectKeys.forEach(function(select) {
    if (attributes[select.key] && attributes[select.key].type.indexOf('geometry') === 0) {
      query += 'ST_asGeoJSON(' + utils.escapeName(select.table) + '.' + utils.escapeName(select.key) + ') as ' + utils.escapeName(select.key) + ', ';
    } else {
